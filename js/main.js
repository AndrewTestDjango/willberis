...
//const postData = dataUser => fetch('server.pp', {	//вариант для проверки ошибки
	method: 'POST',
	body: dataUser,
})
...
modalForm.addEventListener('submit', event => {
	event.preventDefault(); // чтобы не было  станадратной перезагрузки страницы
	const formData = new FormData(modalForm);
	//console.log(formData); //пусто
	//console.log(JSON.stringify.formData); //пусто
	//postData("Hello world!");
	...
	.then(response => {
		if(!response.ok) {
			throw new Error(response.status); // после throw срабатывает catch
		}
		alert('Ваш заказ успешно отправлен. С вами свяжутся в ближайшее время');
		console.log(response.statusText);
		console.log(response.status);
	}) // отрабатывает положительный ответ
	.catch(err => {
	...
	.finally(() => {   //  EC2019
		...
		modalForm.reset(); // стандартная очистка формы
		//cart.cartGoods.length = 0; // простой но не оченб правильный метод
		cart.clearCart(); // Более правильный вариант
	}); 
});
